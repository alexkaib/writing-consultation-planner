...
if (isset($post_data->jwt)) {
  ...
  try {
    ...
    $tutor_info = $post_data->tutorInfo;

    if (isset($tutor_info->password)) {

      // create pw hash
      $pw = password_hash($tutor_info->password, PASSWORD_DEFAULT);

      $sql = "UPDATE peerTutors SET firstName=?, lastName=?, email=?, password=?, subjects=?, role=?, mailText=?
              WHERE ptId=?";

      $stmt = $db_conn->prepare($sql);
      $stmt->bind_param("sssssssi",
        $tutor_info->firstName,
        $tutor_info->lastName,
        $tutor_info->email,
        $pw,
        $tutor_info->subjects,
      ...
      $sql = "UPDATE peerTutors SET firstName=?, lastName=?, email=?, subjects=?, role=?, mailText=?
              WHERE ptId=?";
    ...
    if (!($stmt->error)) {
      echo json_encode([
          "success" => 1,
          "msg" => "Tutor info update successful",
      ]);
    } else {
      echo json_encode([
          "success" => 0,
